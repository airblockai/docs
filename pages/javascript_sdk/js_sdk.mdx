...
airblock.track(event_type, [{event_properties}, [{user_properties}]])


event structure (api_key, ip_addr, uuid, event_time, event_type, event_properties, user_properties)

track(DEFAULT_SESSION_START_EVENT, undefined, {
        sdk_ver: library,
...
# Tracking Events
The SDK tracks default events and  
...
### Metamask Wallets
The SDK sends `metamask_wallets` event when a user changes her blockchain in her Metamask wallet.
...
### Coinbase Wallets
The SDK sends `coinbase_wallets` event when a user changes her blockchain in her Coinbase wallet.
